...
export default function LoginForm() {
  ...
  const { values, errors, touched, handleChange, handleBlur, handleSubmit } =
    useFormValidation<LoginFormData>(
      { email: '', password: '' },
      FORM_VALIDATION_RULES.login as any,
      handleLoginLogic
    );
  ...
  return (
    <div className="w-full max-w-md mx-auto relative z-10 px-4 py-8 sm:p-10 bg-base-100/80 backdrop-blur-md rounded-3xl shadow-2xl border border-base-200">
      <div className="text-center mb-8">
        <h2 className="text-3xl font-extrabold mb-2 bg-gradient-to-r from-primary to-accent bg-clip-text text-transparent">Welcome Back</h2>
        <p className="text-base-content/60">Sign in to continue your learning journey</p>
      </div>
      ...
      <div className="flex flex-col gap-3 mb-6">
        <Button variant="outline" className="w-full flex items-center justify-center gap-2 border-base-300 hover:border-primary" onClick={() => loginWithGoogle()}>
          ...
          <span className="font-semibold text-base-content/80">Continue with Google</span>
        </Button>
      </div>
      ...
      <form onSubmit={handleLoginSubmit} className="flex flex-col gap-5">
        ...
        <div className="flex flex-col gap-1">
          <div className="flex justify-between items-center ml-1">
            <label htmlFor="password" className="text-sm font-bold text-base-content/80">Password</label>
            <Link to="/forgot-password" className="text-xs font-bold text-primary hover:text-primary-focus underline transition-colors">
              Forgot password?
            </Link>
          </div>
          <Input
        ...
        <Button type="submit" variant="primary" className="w-full mt-2 shadow-lg hover:shadow-primary/40 transition-shadow" disabled={authLoading}>
          {authLoading ? <span className="loading loading-spinner loading-sm"></span> : 'Sign In To Account'}
        </Button>
      </form>

      <div className="text-center mt-6 pt-6 border-t border-base-200">
        <p className="text-sm text-base-content/70 mb-3 font-medium">Don't have an account?</p>
        <Link to="/signup">
          <Button variant="outline" className="w-full font-bold">Create new account</Button>
        </Link>
      </div>

      <div className="mt-8 bg-base-200/50 p-4 rounded-xl border border-base-300 text-center">
        <p className="text-xs font-bold text-base-content mb-2 uppercase tracking-wider">Demo Credentials</p>
        <p className="text-sm font-medium text-base-content/80">Email: <span className="text-primary tracking-wide">demo@example.com</span></p>
        <p className="text-sm font-medium text-base-content/80">Password: <span className="text-primary tracking-wide">Demo@123</span></p>
      </div>
    </div>
  );
}
