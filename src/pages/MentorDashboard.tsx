...
export default function MentorDashboard() {
  ...
  const [newCourse, setNewCourse] = useState({ title: '', description: '', price: 0, category: '', image: '' });
  ...
  const fetchMyCourses = async () => {
    try {
      ...
      if (data.success) {
        const myCourses = data.data.filter((course: any) => course.instructor === user?._id || (course.instructor && course.instructor._id === user?._id));
  ...
  const handleCreateCourse = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await fetch('/api/v1/courses', {
        ...
        body: JSON.stringify({ ...newCourse, subjectId: newCourse.title.toLowerCase().replace(/ /g, '-') })
      });
      const data = await res.json();
      if (data.success) {
        setNewCourse({ title: '', description: '', price: 0, category: '', image: '' });
        fetchMyCourses();
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
                <Card className="bg-base-100 border border-base-300 shadow-sm relative overflow-hidden group">
                   ...
                   <CardBody className="p-6">
                     ...
                     <div className="text-4xl font-black">{mentorStats.totalStudents.toLocaleString()}</div>
                     <div className="text-success text-sm font-semibold mt-2 flex items-center gap-1">+12% this month</div>
                   </CardBody>
                </Card>
                ...
                <Card className="bg-base-100 border border-base-300 shadow-sm relative overflow-hidden group">
                   ...
                   <CardBody className="p-6">
                     ...
                     <div className="text-4xl font-black">{mentorStats.rating}</div>
                     <div className="text-base-content/60 text-sm font-semibold mt-2 flex items-center gap-1">Across {courses.length} courses</div>
                   </CardBody>
                </Card>
            ...
            <div className="animate-fadeIn max-w-3xl">
              <Card className="bg-base-100 border border-base-300 shadow-sm">
                <CardHeader className="border-b border-base-200 pb-4">
                  ...
                  <p className="text-base-content/60 text-sm">Fill in the details below to publish a new course.</p>
                </CardHeader>
                <CardBody className="pt-6">
                   <form onSubmit={handleCreateCourse} className="flex flex-col gap-6">
                     ...
                     <div className="flex flex-col gap-2">
                      ...
                      <textarea 
                        rows={5} 
                        className="textarea textarea-bordered w-full text-base focus:outline-none bg-base-100"
                     ...
                     <div className="flex flex-col gap-2 p-6 border border-base-300 rounded-xl bg-base-200/50 mt-2">
                       <label className="font-bold border-b border-base-300 pb-2 mb-2 flex items-center gap-2">
                         <ImageIcon className="w-5 h-5"/> Course Thumbnail
                       </label>
                       <div className="flex items-center gap-6">
                         {newCourse.image ? (
                           ...
                           <div className="h-32 w-48 rounded-lg border-2 border-dashed border-base-300 flex items-center justify-center text-base-content/40 bg-base-100">
                              No image
                           </div>
                         )}
                         <div className="flex-1">
                           <p className="text-sm text-base-content/60 mb-4">Upload a high-quality 16:9 image to attract more students.</p>
                           <CloudinaryUploadWidget 
                             cloudName="dgkzstbui"
                             uploadPreset="e-learning"
                             onUploadSuccess={(url) => setNewCourse({...newCourse, image: url})}
                           />
                         </div>
                       </div>
                     </div>
                     ...
                     <div className="flex justify-end gap-4 mt-6">
                        <Button variant="outline" type="button" onClick={() => setActiveTab('overview')}>Cancel</Button>
                        <Button variant="primary" type="submit" className="shadow-md px-8"><PlusCircle className="w-4 h-4 mr-2"/> Publish Course</Button>
                     </div>
                   </form>
                </CardBody>
              </Card>
                    ...
                    <div key={course._id} className="bg-base-100 border border-base-300 p-4 rounded-xl flex flex-col sm:flex-row justify-between items-start sm:items-center gap-6 shadow-sm hover:shadow-md transition-all">
                      <div className="flex gap-4 items-center w-full sm:w-auto">
                        <div className="w-24 h-16 rounded-md overflow-hidden bg-base-200 shrink-0">
                           {course.image ? <img src={course.image} className="w-full h-full object-cover"/> : <div className="w-full h-full flex items-center justify-center text-base-content/30"><ImageIcon className="w-6 h-6"/></div>}
                        </div>
                        <div>
                      ...
                      <div className="flex gap-2 w-full sm:w-auto mt-4 sm:mt-0">
                        <Button variant="outline" size="sm" className="flex-1 sm:flex-none">Edit Content</Button>
                        <Button variant="danger" size="sm" className="flex-1 sm:flex-none">Unpublish</Button>
                      </div>
                    </div>
                  ))}
                </div>
